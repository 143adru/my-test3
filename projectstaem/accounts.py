...
class AccountInformation(object):
    ...
    def create_account(self, account_type):
        if account_type == "Savings":
            self.account_dict[account_type] = SavingAccount(self.name, self.address)
        elif account_type == "Current":
            self.account_dict[account_type] = CurrentAccount(self.name, self.address)
        elif account_type == "PPF":
            self.account_dict[account_type] = PPFAccount(self.name, self.address)
        elif account_type == "NPS":
            self.account_dict[account_type] = NPSAccount(self.name, self.address)
        elif account_type == "PF":
            self.account_dict[account_type] = PFAccount(self.name, self.address, self.company_name)

    def get_balance(self):
        total_balance = 0
        for account in self.account_dict:
            total_balance += self.account_dict[account].balance

        return total_balance

    def add_amount(self, account_type, amount):
        account_info = self.account_dict[account_type]
    ...
    def get_account_information(self):
        ...
        for account in self.account_dict:
            complete_info += "{} balance is {}\n".format(account, self.account_dict[account].balance)
...
# print(person1.get_account_dict())
